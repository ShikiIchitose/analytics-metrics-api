...
`GET /users/{user_id}` は、`data/clean/users.parquet` が存在する場合はそちらを優先して参照します。`users.parquet` が存在しない場合、または対象ユーザーがそこに存在しない場合は、`data/clean/events.parquet` の最初のイベント行から導出した値にフォールバックします。
...
現在のブラウザデモでは、次を小さな入力フォームから試せます。

- `GET /metrics/{name}` による metric 実行
- `GET /users/{user_id}` による user lookup
- `GET /jobs/runs` による job run 一覧取得
- `GET /jobs/{job_name}/summary` による one-job summary 取得

UI は引き続き意図的に薄い構成のままにしており、既存 API をブラウザから確認しやすくするための補助導線として位置づけています。業務ロジックの中心は backend 側に残しています。
...
このリポジトリは、意図的にスコープを絞っています。

現行の v0.1.x 系には含めていないもの:

- 認証 / 認可
- キャッシュ
- バックグラウンドジョブ / オーケストレーション
- マルチテナント設計
- 書き込み系エンドポイント（`POST`, `PUT`, `PATCH`, `DELETE`）

これらは妥当な次のステップですが、このポートフォリオ用リポジトリで示したいコアなエンジニアリング上のシグナルを示すためには必須ではありません。
